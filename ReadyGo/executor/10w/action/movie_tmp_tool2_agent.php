...
// $short_pi = "3";
// $my_pi = number_format($short_pi, 1);
// echo $my_pi."\n";   // 3.14
// die;
        ...
        // do {
            ...
            $start = 70000;
            show_msg($start.".......<br />\r\n");
            $sql = "select * from movie WHERE douban_id != '' AND douban_image != '' AND douban_image NOT LIKE '%movie-default%' AND year !='' AND year > 0 ORDER BY id ASC LIMIT ".$start.", ".$offet;
            show_msg($sql.".......<br />\r\n");
            $aTmp = $this->gaTools['mysqldb']->find($sql);
            // dump($aTmp);
            if ($aTmp) {
                foreach ($aTmp as $key => $value) {
                    $row = array();
                    $row['mid'] = $value['id'];
                    $row['mtime_id'] = $value['mtime_id'];
                    $row['douban_id'] = $value['douban_id'];
                    $row['imdb_id'] = $value['imdb_id'];
                    $row['title'] = $value['title'];
                    $row['author'] = $value['author'];
                    $row['image'] = str_replace("spic", "mpic", $value['douban_image']);
                    $row['summary'] = $value['summary'];
                    $row['year'] = $value['year'];
                    $row['movie_duration'] = $value['movie_duration'];
                    $row['country'] = $value['country'];
                    $row['writer'] = $value['writer'];
                    $row['director'] = $value['director'];
                    $row['pubdate'] = $value['pubdate'];
                    $row['aka'] = $value['aka'];
                    $row['movie_type'] = $value['movie_type'];
                    ...
                    $aTmp2 = $this->gaTools['mysqldb']->get('SELECT * FROM movie_show WHERE mid='.$row['mid']);
                    if ($aTmp2) {
                        show_msg('<'.$row['title'].">已存在<br />\r\n");
                        $row['id'] = $aTmp2['id'];
                        $r = $this->gaTools['mysqldb']->update('movie_show', $row);
                        continue;
                    } else {
                        $r = $this->gaTools['mysqldb']->save('movie_show', $row);
                    }
                    show_msg('<'.$row['title'].">过滤保存成功<br />\r\n");
                }
            }
            $i++;
        // } while ( $start <= 195560 );
