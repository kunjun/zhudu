...
class mysqldb_tool{
    ...
    public function __construct($c){
    	$this->aInC = $c;
        $this->initDB($this->aInC);
    }
    
    public function initDB($c){
    	if(!isset($c['port'])){
            $c['port'] = '3306';
        }
        ...
        if (!isset($c['mysql_fetch_type']) || $c['mysql_fetch_type'] == 'assoc') {
        	$this->sMysqlFetchType = 'mysql_fetch_assoc';
        } elseif ($c['mysql_fetch_type'] == 'array') {
        	$this->sMysqlFetchType = 'mysql_fetch_array';
        } elseif ($c['mysql_fetch_type'] == 'object') {
        	$this->sMysqlFetchType = 'mysql_fetch_object';
        }
    }
    ...
    public function get($sql){
        ...
        $sMysqlFetchType = $this->sMysqlFetchType;
        if($row = $sMysqlFetchType($result)){
            return $row;
        }else{
            return null;
        }
    ...
    public function find($sql, $key=null){
        ...
        $result = $this->query($sql);
        $sMysqlFetchType = $this->sMysqlFetchType;
        while($row = $sMysqlFetchType($result)){
            if(!empty($key)){
                $data[$row->{$key}] = $row;
            }else{
                $data[] = $row;
